...
def deal_with_objective(objective, pred, truth, args):
    ...
    if type(objective) == nn.CrossEntropyLoss:
        ...
        type(objective) == nn.MSELoss
        or type(objective) == nn.modules.loss.BCEWithLogitsLoss
        or type(objective) == nn.L1Loss
    ):
...
def train(
    ...
    def _trainprocess():
        ...
        for epoch in range(total_epochs):
            ...
            if validtime:
                print(
                    "Valid Time: " + format_time(validendtime - validstarttime)
                )
